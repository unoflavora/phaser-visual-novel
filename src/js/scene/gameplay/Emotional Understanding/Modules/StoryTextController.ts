...
export class StoryTextController extends Phaser.GameObjects.Group {
  ...
  constructor(
    ...
    this._text = new Phaser.GameObjects.Text(
      scene,
      this._textBox.gameobject.x -
        this._textBox.gameobject.displayWidth *
          this._textBox.gameobject.originX +
        this._padding,
      this._textBox.gameobject.y -
        this._textBox.gameobject.displayHeight *
          this._textBox.gameobject.originY +
        this._padding * 0.75,
      "Loading...",
      {
    ...
    this.add(this._text);
    this._text.setFontSize(textBox.gameobject.displayHeight * 0.09);
    this._text.setWordWrapWidth(
      this._textBox.gameobject.displayWidth - this._padding * 2,
      true
    );

    this._nextButton = new Text(
      scene,
      this._textBox.gameobject.x +
        this._textBox.gameobject.displayWidth *
          this._textBox.gameobject.originX -
    ...
    this._nextButton.gameobject.setFontSize(
      textBox.gameobject.displayHeight * 0.09
    );
    this.add(this._nextButton.gameobject);

    this._prevButton = new Text(
      scene,
      this._textBox.gameobject.x -
        this._textBox.gameobject.displayWidth *
          this._textBox.gameobject.originX +
        this._padding,
      this._nextButton.gameobject.y,
      "Previous",
      {
        fontFamily: FontAsset.adobe_caslon_pro_bold.key,
        fontSize: "24px",
    ...
    this._prevButton.gameobject.setFontSize(
      textBox.gameobject.displayHeight * 0.09
    );

    var prevButtonClickArea = scene.add.image(
      this._prevButton.gameobject.x,
      this._prevButton.gameobject.y,
      ""
    );
    prevButtonClickArea.setDisplaySize(
      this._prevButton.gameobject.displayWidth * 3,
      this._prevButton.gameobject.displayHeight * 1.8
    );
    ...
    var nextButtonClickArea = scene.add.image(
      this._nextButton.gameobject.x,
      this._nextButton.gameobject.y,
      ""
    );
    nextButtonClickArea.setDisplaySize(
      this._prevButton.gameobject.displayWidth * 3,
      this._prevButton.gameobject.displayHeight * 1.8
    );
  ...
  public LoadTextResponse(
    ...
    this._prevButton.gameobject.removeAllListeners();
    this._previousAvailable =
      responseIndex > 0 || (responseIndex == 0 && paragraphIndex > 0);
    if (this._previousAvailable) {
      this._prevButton.gameobject.once("pointerdown", () => {
        paragraphIndex -= 1;
        this.LoadTextResponse(
          response,
          monologue,
          responseIndex,
          paragraphIndex
    ...
    if (!this._textBox.gameobject.listenerCount("pointerdown")) {
      this._textBox.gameobject.setInteractive({ useHandCursor: true });
      this._textBox.gameobject.on("pointerdown", () =>
        this.handleTextBoxClick(response[responseIndex].text[paragraphIndex])
      );
    }
