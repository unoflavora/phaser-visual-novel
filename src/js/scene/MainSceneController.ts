...
export default class MainSceneController extends Phaser.Scene {
  ...
  public get progress() {
    return this._progressController;
  }
  ...
  async init(finishLoading: Function) {
    ...
    this._backendController.tokenExpiredDate =
  ...
  ): Promise<Response<AuthData> | null> {
    try {
      ...
      if (auth.error == null) {
        if (rememberUser) {
          ...
          localStorage.setItem(
            "tokenExpiredDate",
            auth.data.tokenExpiredDate.toString()
          );
        }
  ...
  private async gameInit() {
    ConsoleHelper.Log(
      this._backendController.token,
      this.backend.tokenExpiredDate
    );
    ...
    if (
      ...
      try {
        ...
        if (initData.data.savedData != null && initData.data.savedData != "") {
          ...
          if (
            this.gameData.progress.emotionalUnderstanding.userResponses == null
          ) {
        ...
        this.gameData.results = this._scoringController.Init(
          this.gameData.results!,
          this.gameData.progress.emotionalUnderstanding.scores
        );
  ...
  public Logout() {
    try {
      ...
      var loadedScene = scenes.filter(
        (scene) =>
          scene.scene.key != SceneInfo.mainScene.key &&
          scene.scene.key != SceneInfo.debugScene.key
      );
    ...
    try {
      ...
      if (e instanceof Error) {
        this.OpenTemplatePopup(
          PopupType.Error,
          Localizations.text.errors.failed_to_save.desc
        );
      }
    }
  }
  ...
  public OpenInfoPopup(
    ...
    this._popupController.openInfoPopup(
      title,
      message,
      iconKey,
      () => this.ClosePopup(),
  ...
  public AddEmotionScore(emotion: number) {
    this.gameData.progress.emotionalUnderstanding.scores.emotion =
      this._scoringController.addEmotionalScore(emotion);
  }

  public AddRespondScore(respond: number) {
    this.gameData.progress.emotionalUnderstanding.scores.response =
      this._scoringController.addRespondScore(respond);
  }

  public AddMinigameScore(
    minigameType: MinigameTypes,
    minigameScore: GameScore
  ) {
    this.gameData.results = this._scoringController.addMinigameScore(
      minigameType,
      minigameScore
    );
  }
  ...
  public async FinishMinigames() {
    ...
    var scoreData: SubmitScoreData = {
      ...
      eU_Performance:
        finalScore[MinigameTypes.EmotionalUnderstanding].performance,
    };
